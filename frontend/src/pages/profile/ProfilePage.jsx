	...
	return (
		<>
			<div className='flex-[4_4_0]  border-r border-green-400 min-h-screen '>
				...
				{!isLoading && !isRefetching && !user && <p className='text-center text-lg mt-4 text-green-400 font-mono'>Developer not found</p>}
					...
					{!isLoading && !isRefetching && user && (
						<>
							<div className='flex gap-10 px-4 py-2 items-center'>
								...
								<div className='flex flex-col'>
									<p className='font-bold text-lg text-green-300 font-mono'>{user?.fullName}</p>
									<span className='text-sm text-green-500 font-mono'>{POSTS?.length} commits</span>
								</div>
							</div>
							{/* COVER IMG */}
							<div className='relative group/cover'>
								...
								<div className='avatar absolute -bottom-16 left-4'>
									<div className='w-32 rounded-full relative group/avatar border border-green-400'>
										<img src={profileImg || user?.profileImg || "/avatars/boy5.jpg"} />
							...
							<div className='flex justify-end px-4 mt-5'>
								...
								{!isMyProfile && (
									...
									>
										{isPending && "Loading..."}
										{!isPending && amIFollowing && "git unfollow"}
										{!isPending && !amIFollowing && "git follow"}
									</button>
								)}
								{(coverImg || profileImg) && (
									<button
							...
							<div className='flex flex-col gap-4 mt-14 px-4'>
								<div className='flex flex-col'>
									<span className='font-bold text-lg text-green-300 font-mono'>{user?.fullName}</span>
									<span className='text-sm text-green-500 font-mono'>@{user?.username}</span>
									<span className='text-sm my-1 text-green-200 font-mono'>{user?.bio}</span>
								</div>
								...
								<div className='flex gap-2 flex-wrap'>
									...
									<div className='flex gap-2 items-center'>
										...
										<span className='text-sm text-green-500 font-mono'>Joined {memberSinceDate}</span>
									</div>
								</div>
								<div className='flex gap-2'>
									<div className='flex gap-1 items-center'>
										<span className='font-bold text-xs text-green-300 font-mono'>{user?.following.length}</span>
										<span className='text-green-500 text-xs font-mono'>Following</span>
									</div>
									<div className='flex gap-1 items-center'>
										<span className='font-bold text-xs text-green-300 font-mono'>{user?.followers.length}</span>
										<span className='text-green-500 text-xs font-mono'>Followers</span>
									</div>
								</div>
							</div>
